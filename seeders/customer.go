...
func customersSeeder() {
	...
	go func() {
		...
		stmt, err := tx.Prepare("INSERT INTO " + configurations.Configuration().CustomerTableName + " ( customer_id, customer_unique_id, customer_zip_code_prefix, customer_city, customer_state) VALUES (?, ?, ?, ?, ?)")
		if err != nil {
			panic(err)
		}
		defer stmt.Close()
